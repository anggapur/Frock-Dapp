...
import { USDC, CommunityOffering, CommunityOfferingNRT, Frock } from '@project/contracts/typechain/generated';
...
describe("Community Offering", async () => {     
    ...
    before(async () => {
        ...
        // Community Offering
        communityOffering = await ethers.getContract<CommunityOffering>('CommunityOffering');
        nrt = await ethers.getContract<CommunityOfferingNRT>('CommunityOfferingNRT');
    ...
    it('Send Frock to Community Sales', async() => {
        ...
        await frock.connect(deployer).transfer(communityOffering.address, amountFrockToSale)

        const frockBalanceOfCommunitySalesContract = await frock.balanceOf(communityOffering.address)
    ...
    it('Whitelist & Remove Whitelist Users', async() => {
        // Whitelist user1
        await communityOffering.connect(deployer)
        .addWhitelist(user1.address)
    ...
    it('Fail to Invest because Sales not enabled yet', async() => {
        ...
        // Time travel
        const startTime = parseInt((await communityOffering.startTime()).toString())
    ...
    it('Enable Sale', async() => {
        ...
        await expect(
            ...
            .connect(deployer)
            .enableSale()
        ).to.emit(communityOffering, "SaleEnabled")
    ...
    it('Invest', async() => {    
        ...
        await usdc.connect(user3).approve(communityOffering.address, reinvestAmount3)        
        ...
        await usdc.connect(user4).approve(communityOffering.address, reinvestAmount4)        
        await expect(
        ...
        const endTime = parseInt((await communityOffering.endTime()).toString()) + 60
        await network.provider.send('evm_setNextBlockTimestamp', [
            endTime
        ]);
        ...
        await usdc.connect(user3).approve(communityOffering.address, amountToInvest)
        await expect(
    ...
    it('Enable Redeem', async() => {
        ...
        await expect(
            ...
            .connect(deployer)
            .enableRedeem()
        ).to.emit(communityOffering, "RedeemEnabled")
    ...
    it('Redeem', async() => {            
        ...
        await expect(
            communityOffering.connect(user1).redeem()
        ...
        await expect(
            communityOffering.connect(user2).redeem()
        ).to.emit(communityOffering, "Redeem")
        ...
        await expect(
            communityOffering.connect(user3).redeem()
        ).to.emit(communityOffering, "Redeem")
        ...
        // No Amount to redeem
        await expect(
            communityOffering.connect(user4).redeem()
        ).to.be.revertedWith('no amount issued')
    })
    ...
    it('Withdraw Treasury', async() => {                
        ...
        const communityOfferingBalanceBefore = await usdc.balanceOf(communityOffering.address)   
        await communityOffering.connect(deployer).withdrawTreasury(communityOfferingBalanceBefore);
        const treasuryBalanceAfter = await usdc.balanceOf(treasury.address)
        const communityOfferingBalanceAfter = await usdc.balanceOf(communityOffering.address)   
        ...
        expect(treasuryBalanceAfter).to.be.eq(treasuryBalanceBefore.add(amountCollected))
    })
    ...
    it('Withdraw Launch Token', async() => {
        ...
        const remainingFrock = ethers.utils.parseUnits('111250', (await frock.decimals()))
        const balanceFrockOfDeployerBefore = await frock.balanceOf(deployer.address)
        await communityOffering.connect(deployer).withdrawLaunchtoken(remainingFrock);
        const balanceFrockOfCommunityContract = await frock.balanceOf(communityOffering.address)
        ...
        expect( balanceFrockOfDeployerAfter).to.be.eq(balanceFrockOfDeployerBefore.add(remainingFrock))
    })
